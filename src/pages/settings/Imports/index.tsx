...
import SuccessFailureComponent from "./StatusComponent";
...
const ImportCSV = () => {
	...
	useEffect(() => {
		console.log("Updated upload_response Data:", importData.import_status);
		console.log("Updated request ID:", importData.request_id);
	}, [importData.import_status, importData.request_id]);
				...
				onSuccess: (res) => {
					...
					if (res?.request_id) {
						...
						console.log(
							...
							importData.upload_response
						);
						console.log(
							"request id of import data:",
							importData.request_id
						);

						// dispatch(ImportUploadStatus(res));
						dispatch(ImportUploadStatus(res)).then((response) => {
							if (response) {
								dispatch(
									InputChangeValue({
										key: "import_status",
	...
	const handleNextStep = () => {
		if (currentStep === 0) {
			...
			const payload = {
				...responseData,
				import_file_name: importResponseData?.initial?.import_file_name,
				original_file_name:
					importResponseData?.initial?.original_file_name,
				input_format:
								...
								{importResponseData?.has_errors &&
									currentStep == 0 && (
										<Button
											variant="contained"
											color="primary"
											onClick={handleNextStep} // Call the updated function
											disabled={loading}>
											{loading ? (
