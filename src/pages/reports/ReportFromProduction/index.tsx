...
interface SavePayload {
	...
	productionreceipt: miniTypes;
	batch: miniTypes;
	item: miniTypes;
	unit: miniTypes;
}

const ProductionReceiptItemReport = () => {
	const dispatch = useAppDispatch();
	const {
		mini: { productionReport, miniItemsList, miniBatch, miniUnits },
				...
				downloadExportData({
					...
					ParamsData: {
						...
						end_date: end_date
							...
							: "",
						productionreceipt: productionreceipt?.value,
						batch: batch?.value,
						item: item?.value,
						unit: unit?.value,
					},
	...
	return (
		<GoBack
			is_goback={true}
			go_back_url={"/reports"}
			title={`Production Receipt Report`}
						...
						<Grid container spacing={2}>
							...
							<Grid
								...
								<SelectComponent
									...
									control={control}
									rules={{ required: true }}
									options={productionReport?.list.map(
										(e: {
											id: string | number;
											code: string;
										}) => ({
									...
									loading={productionReport?.loading}
									selectParams={{
										page: productionReport?.miniParams
											?.page,
										page_size:
											productionReport?.miniParams
												?.page_size,
										search: productionReport?.miniParams
											?.search,
										no_of_pages:
											productionReport?.miniParams
												?.no_of_pages,
									...
									hasMore={
										productionReport?.miniParams?.page <
										productionReport?.miniParams
											?.no_of_pages
									}
									fetchapi={getMiniProductionReceipt}
									clearData={clearMiniProductionReceipt}
								/>
							</Grid>
...
export default ProductionReceiptItemReport;
