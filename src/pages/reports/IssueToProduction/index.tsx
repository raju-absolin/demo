...
interface SavePayload {
	...
	issuetoproduction: miniTypes;
	batch: miniTypes;
	item: miniTypes;
	unit: miniTypes;
}

const IssueToProductionItemReport = () => {
	const dispatch = useAppDispatch();
	const {
		mini: { issueToProduction, miniItemsList, miniBatch, miniUnits },
	...
	const onFinish = ({
		...
		var objData = {
			...
			model_name: "IssueToProductionItem",
		};
				...
				downloadExportData({
					...
					ParamsData: {
						...
						end_date: end_date
							...
							: "",
						issuetoproduction: issuetoproduction?.value,
						batch: batch?.value,
						item: item?.value,
						unit: unit?.value,
					},
	...
	return (
		<GoBack
			is_goback={true}
			go_back_url={"/reports"}
			title={`Issue To Production Report`}
						...
						<Grid container spacing={2}>
							...
							<Grid
								...
								<SelectComponent
									...
									control={control}
									rules={{ required: true }}
									options={issueToProduction?.list.map(
										(e: {
											id: string | number;
											code: string;
										}) => ({
									...
									loading={issueToProduction?.loading}
									selectParams={{
										page: issueToProduction?.miniParams
											?.page,
										page_size:
											issueToProduction?.miniParams
												?.page_size,
										search: issueToProduction?.miniParams
											?.search,
										no_of_pages:
											issueToProduction?.miniParams
												?.no_of_pages,
									...
									hasMore={
										issueToProduction?.miniParams?.page <
										issueToProduction?.miniParams
											?.no_of_pages
									}
									fetchapi={getMiniIssuetoproduction}
									clearData={clearMiniIssueToProduction}
								/>
							</Grid>
							...
							<Grid size={{ xs: 12, md: 4, xl: 2 }}>
								<SelectComponent
									...
									selectParams={{
										...
										page_size:
											miniBatch.miniParams.page_size,
...
export default IssueToProductionItemReport;
