...
const MaterialRequest = () => {
	...
	const {
		materialRequest: { mrList, pageParams, mrCount, loading, isFilterOpen },
		...
		return mrList?.map((row, key) => {
							...
							<Chip
								...
								color={(() => {
									let tagColor:
										...
										| "info"
										| "warning" = "default";
									switch (row.authorized_status_name) {
										case "Approved":
											tagColor = "success";
											break;
										case "Pending":
			...
			const code = (
				<Tooltip
					...
					title="Click to view material request details">
					<Link
						to={`/work_order/view/${id}/${tab}/project/material_request/view/${row.id}`}>
						<Button
							color="primary"
							variant="contained"
							sx={{
					...
					{userAccessList?.indexOf(
						...
						row?.authorized_status_name != "Rejected" && (
							<Tooltip
								...
								<Link
									to={`/work_order/view/${id}/${tab}/project/material_request/${row.id}`}>
									<LuPencil
										style={{
											cursor: "pointer",
											color: "#fc6f03",
	...
	return (
		<Box>
			<TopComponent
				permissionPreFix="MaterialRequest"
				permissionPostFix="materialrequest"
				navigateLink={`/work_order/view/${id}/${tab}/project/material_request/0`}
				showAddButton={true}
				showFilterButton={true}
				openFilter={handleFilter}
				addButtonName="Add Material Request"
