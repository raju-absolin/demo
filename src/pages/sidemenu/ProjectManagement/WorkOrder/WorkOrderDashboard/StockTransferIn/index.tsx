...
const StockIn = () => {
	...
	const rows = useMemo(() => {
		return stockInList?.map((row, key) => {
			const index =
			...
			const code = (
				<Tooltip
					...
					title="Click to view Stock Transfer In details">
					<Link
						to={`/work_order/view/${id}/${tab}/project/stock_transfer_in/view/${row.id}`}>
						<Button
							color="primary"
							variant="contained"
							sx={{
			...
			const actions = (
				<Box
					...
					{userAccessList?.indexOf(
						"StockTransfer.change_stocktransferin"
					) !== -1 && (
						<Tooltip
							TransitionComponent={Zoom}
							title="Edit Stock Transfer In">
							<Link
								to={`/work_order/view/${id}/${tab}/project/stock_transfer_in/${row.id}`}>
								<LuPencil
									style={{
										cursor: "pointer",
										color: "#fc6f03",
	...
	return (
		<Box>
			<TopComponent
				...
				permissionPostFix="stocktransferin"
				navigateLink={`/work_order/view/${id}/${tab}/project/stock_transfer_in/0`}
				showAddButton={true}
				showFilterButton={true}
				openFilter={handleFilter}
				addButtonName="Add Stock Transfer In"
