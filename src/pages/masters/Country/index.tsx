...
import {
	...
	getCountry,
	getCountryById,
} from "@src/store/masters/Country/country.action";
import {
	countrySelector,
	setSearchValue,
	isModelVisible,
	...
	setMasterEditId,
	setSelectedData,
} from "@src/store/masters/Country/country.slice";
...
import AddCountryMasters from "./add.country";
...
const Country = () => {
	...
	const rows = countryList.map((row, key) => {
		...
		const actions = (
			<Box
				...
				{userAccessList?.indexOf("DynamicDjango.change_country") !==
					-1 && (
					<LuPencil
						style={{ cursor: "pointer", color: "#fc6f03" }}
						onClick={() => {
							dispatch(getCountryById({ id: row?.id }));
							openModal(true);
							dispatch(setMasterEditId(row?.id));
						}}
					/>
				)}
				{userAccessList?.indexOf("DynamicDjango.delete_country") !==
					-1 && (
					<>
						<LuTrash2
							style={{ cursor: "pointer", color: "#fc6f03" }}
	...
	useEffect(() => {
		...
		outletContext?.setTitle("Country");
	...
	return (
		<>
			<MasterPageTitle
				...
				goBack={true}
				addModelTile={
					userAccessList?.indexOf("DynamicDjango.add_country") !== -1
						? "Add Country"
						: undefined
				}
				modalVisible={true}
