...
import {
	...
	getDocument,
	getDocumentById,
} from "@src/store/masters/Document/document.action";
import {
	documentSelector,
	setSearchValue,
	isModelVisible,
	...
	setMasterEditId,
	setSelectedData,
} from "@src/store/masters/Document/document.slice";
...
import AddMasters from "./add.document";
...
const Document = () => {
	...
	const rows = documentList.map((row, key) => {
		...
		const actions = (
			<Box
				...
				{userAccessList?.indexOf("Masters.change_document") !== -1 && (
					<LuPencil
						style={{ cursor: "pointer", color: "#fc6f03" }}
						onClick={() => {
							dispatch(getDocumentById({ id: row?.id }));
							openModal(true);
							dispatch(setMasterEditId(row?.id));
						}}
					/>
				)}
				{userAccessList?.indexOf("Masters.delete_document") !== -1 && (
					<>
						<LuTrash2
							style={{ cursor: "pointer", color: "#fc6f03" }}
	...
	useEffect(() => {
		...
		outletContext?.setTitle("Document");
	...
	return (
		<>
			<MasterPageTitle
				...
				goBack={true}
				addModelTile={
					userAccessList?.indexOf("Masters.add_document") !== -1
						? "Add Document"
						: undefined
				}
				modalVisible={true}
