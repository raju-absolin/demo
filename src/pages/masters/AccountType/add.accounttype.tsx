...
// import { addAccountTypess, getAccountTypess, editAccountTypess, getAccountTypessById } from "@src/store/masters/AccountTypes/accountType.action";
import {
	accountTypeSelector,
	isModelVisible,
	setMasterValue,
...
} from "@src/store/masters/AccountType/accountType.slice";
import {
	addAccountTypes,
	editAccountTypes,
} from "@src/store/masters/AccountType/accountType.action";
...
const AddAccountTypesMasters = () => {
	...
	const accountTypesSchema = yup.object().shape({
		name: yup
			.string()
			.required("Please enter your accountType name")
			.trim()
			.matches(
				/^[a-zA-Z0-9 ]*$/,
				"accountType name should not contain special characters"
			),
		remarks: yup
			.string()
	...
	const { control, handleSubmit, reset } = useForm<any>({
		resolver: yupResolver(accountTypesSchema),
		values: {
	...
	const onSubmit = (data: any) => {
		if (!edit_Id) {
			...
			dispatch(addAccountTypes(payload));
		} else {
			const obj = {
				id: edit_Id,
				name: data.name,
			...
			dispatch(editAccountTypes(payload));
		}
	};
	return (
		<>
								...
								<Grid container spacing={2}>
									<Grid size={{ xs: 24 }}>
										<FormInput
											...
											label="Name"
											type="text"
											placeholder="Enter AccountTypes here..."
											control={control}
										/>
									</Grid>
									{/* <Grid size={{ xs: 24 }}>
...
export default AddAccountTypesMasters;
