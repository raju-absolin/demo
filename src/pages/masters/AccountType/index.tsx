...
import AddAccountTypeMasters from "./add.accounttype";
...
const AccountTypes = () => {
	...
	const rows = accountTypesList.map((row, key) => {
		...
		const actions = (
			<Box
				...
				{userAccessList?.indexOf("DynamicDjango.change_accounttype") !==
					-1 && (
					<LuPencil
						style={{ cursor: "pointer", color: "#fc6f03" }}
						onClick={() => {
							dispatch(getAccountTypesById({ id: row?.id }));
							openModal(true);
						}}
					/>
				)}
				{userAccessList?.indexOf("DynamicDjango.delete_accounttype") !==
					-1 && (
					<>
						<LuTrash2
							style={{ cursor: "pointer", color: "#fc6f03" }}
				...
				{/* <Link to={`/pages/masters/accountTypes/` + row.id}>
	...
	useEffect(() => {
		...
		outletContext?.setTitle("AccountTypes");
	...
	return (
		<>
			...
			<MasterPageTitle
				...
				goBack={true}
				addModelTile={
					userAccessList?.indexOf("DynamicDjango.add_accounttype") !==
					-1
						? "Add AccountType"
						: undefined
				}
