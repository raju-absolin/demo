...
import {
	...
	getTax,
	getTaxById,
} from "@src/store/masters/Tax/tax.action";
import {
	taxSelector,
	setSearchValue,
	isModelVisible,
	setMasterEditId,
	setSelectedData,
} from "@src/store/masters/Tax/tax.slice";
...
import AddTaxMasters from "./add.tax";
...
const Tax = () => {
	...
	const rows = taxList?.map((row, key) => {
		...
		const actions = (
			<Box
				...
				{userAccessList?.indexOf("Masters.change_tax") !== -1 && (
					<LuPencil
						style={{ cursor: "pointer", color: "#fc6f03" }}
						onClick={() => {
							dispatch(getTaxById({ id: row?.id }));
							openModal(true);
							dispatch(setMasterEditId(row?.id));
						}}
					/>
				)}
				{userAccessList?.indexOf("Masters.delete_tax") !== -1 && (
					<>
						<LuTrash2
							style={{ cursor: "pointer", color: "#fc6f03" }}
		...
		return createData(index, row.code, row.name, row.tax, actions);
	});
	...
	return (
		<>
			<MasterPageTitle
				...
				goBack={true}
				addModelTile={
					userAccessList?.indexOf("Masters.add_tax") !== -1
						? "Add Tax"
						: undefined
				}
				modalVisible={true}
