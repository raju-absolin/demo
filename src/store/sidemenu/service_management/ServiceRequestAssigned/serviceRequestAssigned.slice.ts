...
import { ServiceRequestsAssignedState } from "./serviceRequestAssigned.types";
import {
...
} from "./serviceRequestAssigned.action";
import { systemSelector } from "@src/store/system/system.slice";
import { miniSelector } from "@src/store/mini/mini.Slice";

const initialState: ServiceRequestsAssignedState = {
	loading: false,
...
const serviceRequestSlice = createSlice({
	...
	extraReducers(builder) {
		builder
			.addCase(getServiceRequestsAssigned.pending, (state, action) => {
				state.status = "getServiceRequestsAssigned pending";
				state.loading = true;
			})
			.addCase(getServiceRequestsAssigned.fulfilled, (state, action) => {
				const { response, params } = action.payload;
				state.status = "getServiceRequestsAssigned succeeded";
				state.loading = false;
			...
			.addCase(getServiceRequestsAssigned.rejected, (state, action) => {
				state.status = "getServiceRequestsAssigned failed";
				state.loading = false;
				state.error =
...
export const serviceRequestSelector = (state: RootState) =>
	state.serviceManagement.serviceRequestAssigned;
