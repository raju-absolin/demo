...
import {
	ServiceRequestsAssignedState,
	ServiceRequestsAssigned,
} from "./serviceRequestAssigned.types";
import Swal from "sweetalert2";
import { ApiPayload } from "./serviceRequestAssigned.types";
import { formatErrorMessage } from "../../errorMsgFormat";

type GetServiceRequestsPayloadTypes =
	ServiceRequestsAssignedState["pageParams"] & {
		from_date?: string;
		to_date?: string;
	};

export const getServiceRequestsAssigned = createAsyncThunk<
	{
		response: {
			results: ServiceRequestsAssigned[];
			count: number;
		};
		params: PageParamsTypes;
	},
	GetServiceRequestsPayloadTypes
>("serviceRequest/getServiceRequestsAssigned", async (payload) => {
	var params = addParams(payload);

	try {
		const response = await getParamsList(
			// "/servicerequest/servicerequest/assignlist/",
...
export const getServiceRequestById = createAsyncThunk<
	{
		response: ServiceRequestsAssigned;
	},
	{ id: string }
...
>("serviceRequest/postServiceRequestData", async (payload, { dispatch }) => {
	...
	try {
		...
		if (response.status >= 200 && response.status < 300) {
			hide();
			dispatch(getServiceRequestsAssigned(params));
			Swal.fire({
	...
	async (payload, { dispatch }) => {
		...
		try {
			...
			const response: any = await postEdit(
				`/servicerequest/servicerequest/update/${id}/`,
				{
					...data,
				}
			);
