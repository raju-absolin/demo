...
import { PQItem, Vendor } from "../purchaseEnquiry/purchase_enquiry.types";
...
const purchaseQuotationSlice = createSlice({
	...
	extraReducers(builder) {
		builder
			...
			.addCase(deletePQ.pending, (state, action) => {
				state.status = "deletePQ loading";
				state.loading = true;
			})
			.addCase(deletePQ.fulfilled, (state, action) => {
				state.status = "deletePQ succeeded";
				state.loading = false;
			})
			.addCase(deletePQ.rejected, (state, action) => {
				state.status = "deletePQ failed";
				state.loading = false;
				state.error =
					action.error?.message || "An unknown error occurred";
			});
...
export const purchaseQuotationSelectors = (state: RootState) =>
	state.projectManagement.purchaseQuatation;
