...
>("project/pq/getPurchaseQuotations", async (payload) => {
...
export const getPurchaseEnquiryById = createAsyncThunk<
	...
	{ id: string }
>("project/pq/getPurchaseEnquiryById", async (payload) => {
	const { id } = payload;
	try {
...
export const getPurchaseQuotationById = createAsyncThunk<
	...
	{ id: string }
>("project/pq/getPurchaseQuotationById", async (payload) => {
	const { id } = payload;
	try {
...
export const getVendorByEnquiryId = createAsyncThunk<
	...
	{ id: string }
>("project/pq/getVendorByEnquiryId", async (payload) => {
	const { id } = payload;
	try {
...
>("project/pq/postPurchaseQuotation", async (payload, { dispatch }) => {
...
>("project/pq/editPurchaseQuotation", async (payload, { dispatch }) => {
	...
	try {
		...
		if (response.status >= 200 && response.status < 300) {
			...
			Swal.fire({
				title: `<p style="font-size:20px">Success</p>`,
				text: "Purchase Quotation Edited Successfully",
				icon: "success",
				confirmButtonText: `Close`,
				confirmButtonColor: "#3085d6",
			}).then(() => {
...
export const getVendorsByPE = createAsyncThunk<any, { id: string }>(
	"project/pq/getVendorsByPE",
	async (payload) => {
		var params = addParams(payload);
		const { id } = params;
		try {
...
>("project/pq/getPQAttachments", async (payload) => {
	var params = addParams(payload);
...
>("project/pq/deletePQAttachment", async (payload, { dispatch }) => {
...
export const deletePQ = createAsyncThunk<
	void,
	{
		id: string;
		params: PurchaseQuotationInitialState["pageParams"];
	}
>("/deletePQ", async (payload, { dispatch }) => {
	try {
		const { id, params } = payload;
		const response2 = await postDelete(`/quotation/quotation/${id}/`);
		if (response2.status == 204) {
			Swal.fire({
				title: `<p style="font-size:20px">Quotation deleted Successfully</p>`,
				icon: "success",
				confirmButtonText: `Close`,
				confirmButtonColor: "#3085d6",
			});
			dispatch(
				getPurchaseQuotations(params)
			);
		} else {
